...
def inference(args, rank, select_test_datas):
    ...
    for data in select_test_datas:
        ...
        if prev_generated_image is None:
            p_img = Image.open(p_img_path).convert("RGB").resize((args.img_weigh, args.img_height), Image.BICUBIC)
#
        else:
            p_img = prev_generated_image.convert("RGB").resize((args.img_weigh, args.img_height), Image.BICUBIC)

        #p_img = Image.open(p_img_path).convert("RGB").resize((args.img_weigh, args.img_height), Image.BICUBIC)
        s_img_t_mask.paste(p_img, (0, 0))
        s_img_t_mask.paste(black_image, (s_img.width, 0))
        
        s_pose_path = p_img_path.replace("/img/", "/pose_img/").replace(".jpg", "_pose.jpg")
        t_pose_path = t_img_path.replace("/img/", "/pose_img/").replace(".jpg", "_pose.jpg")
        ...
        if args.calculate_metrics:
            ...
            prev_generated_image = grid_metric
            ...
            grid.save(
                ...
                t_img_path.split("/")[-1])
        # print("[CHECK] prev image:", p_img.filename if isinstance(p_img, Image.Image) else "generated")

        # print("[CHECK] source image:", s_img_path)
        # print("[CHECK] target image:", t_img_path)
        # print("[CHECK] prev pose:", s_pose_path)
        # print("[CHECK] target pose:", t_pose_path)
