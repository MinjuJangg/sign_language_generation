...
from PCDMs.src.dataset.stage2_only_color import InpaintDataset, InpaintCollate_fn
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    # 학습이 끝난 후 모델을 최종적으로 저장
                    final_checkpoint_path = os.path.join(args.output_dir, "final_model_checkpoint")
                    checkpoint_model(
                        args.output_dir, "final_model_checkpoint", sd_model, epoch, global_steps
                    )
